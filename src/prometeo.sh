...
def mixture(sample):
  print(sample['Flow']['mixture'])
  assert sample['Flow']['mixture'] == 'ConstPropMix' or \
         sample['Flow']['mixture'] == 'AirMix'
  return sample['Flow']['mixture']
f = json.load(open('$1'))
if '$2' == 'single':
  print wallTime(f), numRanks(f), mixture(f)
elif '$2' == 'dual':
  assert mixture(f['configs'][0]) == (f['configs'][1])
