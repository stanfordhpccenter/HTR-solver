...
with h5py.File(name, 'w') as fout:
   with h5py.File(args.hdf_file[0], 'r') as fin:
      ...
      for fld in fin:
         layout.append(h5py.VirtualLayout(shape=shape, dtype=fin[fld].dtype))

   for i in range(num_files):
      with h5py.File(args.hdf_file[i], 'r') as fin:
         for key,val in fin.attrs.items():
            assert (val == fin.attrs.get(key)).all()
         for (j,fld) in enumerate(fin):
            vsource = h5py.VirtualSource(os.path.abspath(args.hdf_file[i]), fld, shape=fin[fld].shape)
            layout[j][lo_bound[i][2]:hi_bound[i][2]+1,
                      lo_bound[i][1]:hi_bound[i][1]+1,
                      lo_bound[i][0]:hi_bound[i][0]+1] = vsource

   with h5py.File(args.hdf_file[0], 'r') as fin:
      for (i,fld) in enumerate(fin):
         fout.create_virtual_dataset(fld, layout[i])
