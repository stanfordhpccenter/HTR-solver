      ...
      for (j,jc) in enumerate(kc):
         for (i,ic) in enumerate(jc):
            ...
            if yGrid[j+lo_bound[1]] < delta99In:
               velocity[k,j,i][0] += RandAmp*UInf*random.uniform(-1.0, 1.0)
               velocity[k,j,i][1] += RandAmp*UInf*random.uniform(-1.0, 1.0)
               velocity[k,j,i][2] += RandAmp*UInf*random.uniform(-1.0, 1.0)
